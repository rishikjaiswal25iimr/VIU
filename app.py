...
against Manganese Metal Briquette (99% Mn).
...
with st.sidebar:
    ...
    if comparison_selection != "Not selected":
        ...
        P_Chill_MCFeMn  = st.slider("MC FeMn Chill Factor (°C/kg/t)", 1.0, 4.0, 1.25, 0.01)
        P_Chill_Briq    = st.slider("Briq Chill Factor (°C/kg/t)", 0.5, 2.5, 1.0, 0.05)
        ...
        P_Alloy_Target = st.number_input("Alloy Addition Rate (kg/T)", value=6.72, step=0.1, min_value=1.0, max_value=20.0, format="%.2f")
        P_LF_Efficiency = st.slider("LF Efficiency (%)",       25.0, 80.0, 50.0, 1.0) / 100
        ...
        R_Power       = st.slider("Power Realization",       0.50, 1.00, 1.00, 0.01)
        R_Electrode   = st.slider("Electrode Realization",   0.50, 1.00, 1.00, 0.01)
        ...
        R_Hydrogen    = st.slider("Hydrogen Penalty Realization", 0.20, 1.00, 1.00, 0.01)
        R_Refractory  = st.slider("Refractory Realization",  0.10, 1.00, 1.00, 0.01)

        st.divider()
        st.markdown("### F. Enterprise Savings")
        Briq_Consumption_FY = st.number_input("Consumption (MT)", value=8300, step=100, min_value=100, max_value=100000)
...
Alloy_MC = P_Alloy_Target
Active_Mn = Alloy_MC * P_MCFeMn_Mn * P_MCFeMn_Rec
Alloy_Briq = Active_Mn / (P_Briq_Mn * P_Briq_Rec)

Steel_Per_MT_Briq = 1000.0 / Alloy_Briq
Scale_Factor = 1000.0 / (Alloy_Briq * P_Heat_Size)  # Converts ₹/heat to ₹/MT Briquette
kWh_MJ = 3.6

# --- Power Saving (Rigorous Mass Balance Basis per MT Briquette) ---
Temp_Drop_MC = P_Chill_MCFeMn * Alloy_MC
Temp_Drop_Briq = P_Chill_Briq * Alloy_Briq
Delta_Temp_Rigorous = Temp_Drop_MC - Temp_Drop_Briq

Energy_Saved_per_T_steel = (Delta_Temp_Rigorous * P_SpHeat_Steel) / (kWh_MJ * P_LF_Efficiency)
Power_kWh_Saved_Per_MT = Energy_Saved_per_T_steel * Steel_Per_MT_Briq
...
# --- Recovery Stability ---
Benefit_Stability = (MCFeMn_Overdose - Briq_Overdose) * P_MCFeMn_Price * R_Stability

# --- Re-treatment Reduction ---
Benefit_Retreatment = (Retreatment_MCFeMn - Retreatment_Briq) * P_LF_Retreatment_Cost * Scale_Factor * R_Reblow

# --- Cleanliness ---
Benefit_Cleanliness = (Reject_MCFeMn - Reject_Briq) * P_Steel_Value * (1000.0 / Alloy_Briq) * R_Cleanliness

# --- Yield Improvement ---
Benefit_Yield = P_Yield_Factor * P_Steel_Value * (1000.0 / Alloy_Briq) * R_Yield

# --- Carbon Correction Avoidance ---
Benefit_Carbon = C_Corr_Freq_MCFeMn * RH_Corr_Time * P_RH_Minute_Cost * R_Carbon * Scale_Factor

# --- Hydrogen Penalty ---
Benefit_Hydrogen = -(H2_Pickup_Briq / H2_Degas_Rate) * P_RH_Minute_Cost * R_Hydrogen * Scale_Factor
...
# Note: Since the benefits are structured per MT Briquette, the Iron Credit Penalty
# must be mapped accurately to the replacement ratio.
Substitution_Ratio = Alloy_Per_MT_Mn_Briq / Alloy_Per_MT_Mn_MC # Briq / MC -> Wait
Substitution_Ratio = Alloy_MC / Alloy_Briq # MT of MC FeMn replaced by 1 MT of Briquette
Lost_Iron_Credit_per_MT_Briq = Iron_Credit_MC * Substitution_Ratio
...
# (If Cost_Per_Mn_Delta is negative, Briquette is cheaper, adding to the savings)
...
all_benefit_basis = [
    f"ΔT={Delta_Temp_Rigorous:.2f}°C/t steel, {P_LF_Efficiency*100:.0f}% LF eff, {R_Power*100:.0f}% real.",
    f"P_kWh_saved={Power_kWh_Saved_Per_MT:.1f} kWh/MT Briq, {P_Graphite_Factor*1000:.0f}g/kWh, {R_Electrode*100:.0f}% real.",
    f"Time saved={Time_Saved_Min:.2f} min/heat, {R_Throughput*100:.0f}% real.",
    f"Overdose Δ={(MCFeMn_Overdose-Briq_Overdose)*100:.1f}%, {R_Stability*100:.0f}% real.",
    ...
    f"C-corr freq={C_Corr_Freq_MCFeMn*100:.0f}%, {RH_Corr_Time}min, ₹{P_RH_Minute_Cost}/min.",
...
heat_matrix = []
# Only realizable benefits included
heat_names = [
    "Power Saving", "Electrode Saving", "Throughput Gain",
    "Recovery Stability", "Re-treatment Reduction",
...
base_heat_values = [
    ...
    (MCFeMn_Overdose - Briq_Overdose) * P_MCFeMn_Price,
    (Retreatment_MCFeMn - Retreatment_Briq) * P_LF_Retreatment_Cost * Scale_Factor,
    (Reject_MCFeMn - Reject_Briq) * P_Steel_Value * (1000.0 / Alloy_Briq),
    P_Yield_Factor * P_Steel_Value * (1000.0 / Alloy_Briq),
    C_Corr_Freq_MCFeMn * RH_Corr_Time * P_RH_Minute_Cost * Scale_Factor,
]
...
# Build waterfall mapping from MC FeMn active cost down to Briquette effective value
wf_labels = [
    "MC FeMn Active Mn Cost",
    "Power Saving",
    "Electrode Saving",
    "Throughput Gain",
    "Recovery Stability",
    ...
    "Refractory Life",
    "Lost Iron Credit",
    "Mn Briquette Active Mn Cost",
]
# We start from Active Mn Cost, apply credits (as offsets mapping towards Briquette equivalent cost)
wf_values = [
    Cost_Per_Mn_MC,           # base
    Benefit_Power,            
    Benefit_Electrode,
    ...
    Benefit_Hydrogen,         
    Benefit_Refractory,
    -Lost_Iron_Credit_per_MT_Briq, # adds back to the premium gap
    0,                        # total placeholder
]
...
wf_colors.append("#4CAF50")  # Briq total
...
with col_a:
    ...
    fig_stack.add_trace(go.Bar(
        ...
        y=[Cost_Per_Mn_MC, Cost_Per_Mn_Briq],
        marker_color=[C_MCFEMN, C_BRIQ],
        text=[f"₹{Cost_Per_Mn_MC:,.0f}", f"₹{Cost_Per_Mn_Briq:,.0f}"],
        textposition="inside",
    ))
    fig_stack.add_trace(go.Bar(
    ...
    fig_stack.add_trace(go.Bar(
        ...
        y=[0, Lost_Iron_Credit_per_MT_Briq],
        marker_color=["rgba(0,0,0,0)", "#FF7043"],
        text=["", f"+₹{Lost_Iron_Credit_per_MT_Briq:,.0f}"],
        textposition="inside",
    ))
...
with col_b:
    # Briquette Price sensitivity on Net VIU
    briq_prices  = np.linspace(P_MCFeMn_Price * 0.8, P_MCFeMn_Price * 2.5, 80)
    # Re-running the math for varying Briquette prices
    ...
    fig_sens.update_layout(
        **_layout("Mn Briquette Price Sensitivity – Net VIU Advantage (₹/MT)", "Net Advantage (₹/MT)", 420)
    )
    st.plotly_chart(fig_sens, use_container_width=True)

st.markdown("<br>", unsafe_allow_html=True)

# --- MC FeMn price sensitivity ---
...
with col_c:
    ...
    fig_mc_sens.add_trace(go.Scatter(
        x=mc_prices, y=net_mc_sens,
        mode="lines", name="Net VIU (varying MC FeMn price)",
        line=dict(color=C_MCFEMN, width=3),
        fill="tozeroy",
    ...
    fig_mc_sens.update_layout(
        **_layout("MC FeMn Price Sensitivity – Net VIU Advantage (₹/MT)", "Net Advantage (₹/MT)", 380)
    )
...
df_cmp = pd.DataFrame({
    ...
    "MC FeMn": [
        f"₹{P_MCFeMn_Price:,}",
        f"{P_MCFeMn_Mn*100:.1f}%",
        f"{P_MCFeMn_Rec*100:.1f}%",
        f"{P_MCFeMn_Mn*P_MCFeMn_Rec*100:.1f}%",
        f"{Alloy_Per_MT_Mn_MC:.3f} MT",
        f"₹{Cost_Per_Mn_MC:,.0f}",
        "—",
        "—",
        f"₹{Cost_Per_Mn_MC:,.0f}",
    ...
    "Mn Briquette": [
        ...
        f"{P_Briq_Mn*100:.1f}%",
        f"{P_Briq_Rec*100:.1f}%",
        f"{P_Briq_Mn*P_Briq_Rec*100:.1f}%",
        f"{Alloy_Per_MT_Mn_Briq:.3f} MT",
        f"₹{Cost_Per_Mn_Briq:,.0f}",
        f"₹{Gross_Op_Benefits:,.0f}",
        f"₹{Lost_Iron_Credit_per_MT_Briq:,.0f}",
        f"₹{Cost_Per_Mn_Briq - Total_Op_Credits:,.0f}",
    ],
}).set_index("Metric")
...
<div style="text-align:center; color:#90A4AE; font-size:12px; padding:8px 0;">
  VIU Dashboard – MC FeMn vs Mn Briquette &nbsp;|&nbsp; All formulas sourced dynamically 
  (INPUT_PARAMETER → BREAKDOWN_CALC → VIU_SUMMARY) &nbsp;|&nbsp; 
  Operational benefits tracked per MT alloy at stated realization factors.
</div>
""", unsafe_allow_html=True)
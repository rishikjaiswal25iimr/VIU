...
# ══════════════════════════════════════════════════════════════════════════════
def _layout(title: str, y_title: str = "", height: int = 440) -> dict:
    return dict(
...
# Store original conversion target variables safely to secure robust rendering
...
# --- Grouping Operational Penalties globally to secure robust cross-section availability ---
penalty_names = [
...
# Enterprise Savings Calculation (Simplified: Directly multiply physical base consumption as specified)
...
# --- Dynamic Comparative Parameters ---
# 1. Al cost gaps (per MT Effective Al)
Cost_Gap_Pri_Sec   = Raw_Cost_Pri - Raw_Cost_Sec
Cost_Gap_Pri_Notch = Raw_Cost_Pri - Raw_Cost_Notch

# Label logic: "Primary Costlier" if +ve, else alternative is costlier
...
# Net savings represent the economic delta. If positive, the alternative (Secondary or Notch) is cheaper.
# If negative, Primary remains cheaper.
...
# 4. Annual Value of substitution without Effective Al conversion logic
# Formula: Net Savings * Base Al Consumption * Substitution_Pct / 1e7
...
# Heatmap parameters structured to secure clear high-contrast display with solid colors
...
with col_lft:
    ...
    # Savings dynamically driven on a physical consumption basis consistent with Enterprise Savings

...
with st.sidebar:
    ...
    if comparison_selection != "Not selected":
        ...
        P_Pri_Yield    = st.slider("Primary Metallic Yield (%)",   99.0, 100.0, 99.90, 0.01) / 100
        ...
        P_Cycle_Time   = st.slider("LF Cycle Time (min)",        30,   90,   45,  1)
        Active_Al      = st.number_input("Al Addition Target (%)", value=0.05, step=0.01, format="%.3f")
        
        P_Pri_Overdose   = st.number_input("Primary Overdose Buffer",   value=0.015, format="%.3f")
        P_Sec_Overdose   = st.number_input("Secondary Overdose Buffer", value=0.020, format="%.3f")
        P_Notch_Overdose = st.number_input("Notch Bar Overdose Buffer", value=0.025, format="%.3f")
        
        P_Pri_Reject     = st.number_input("Primary Rejection Rate",    value=0.0003, format="%.4f")
        P_Sec_Reject     = st.number_input("Secondary Rejection Rate",  value=0.0004, format="%.4f")
        P_Notch_Reject   = st.number_input("Notch Bar Rejection Rate",  value=0.0005, format="%.4f")
        ...
        R_Cleanliness = st.slider("Cleanliness Realization", 0.0, 1.0, 0.60, 0.05)
        ...
        Al_Consumption_FY = st.number_input("Base Al Consumption (MT)", value=10000, step=100, min_value=100)
...
# Based on migrating from Primary Al (Baseline) to the lowest cost option (if cheaper)
# Or, if Primary Al is the best, avoiding the penalty of switching to a cheaper-priced but lower-VIU option.
...
# Compute Savings/MT for the top cards vs Baseline Primary
...
heat_matrix = []
# Base values excluding the realization factor currently applied
base_heat_values = [
...
with col_b:
    ...
    # Total penalties also have a component dependent on Sec Price (Stability Penalty)
    # Pen_Stab_Sec = (Overdose) * (Sec_Price / 1000) * Steel_Sec * R
    # Conv_Pen_Sec = (Fixed_Penalties + Variable_Penalty) * Req_Sec
    ...
    net_viu_secs = Adj_Cost_Pri - adj_cost_secs  # Positive means Sec is cheaper

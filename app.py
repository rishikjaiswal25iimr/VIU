...
pSlagCost = st.sidebar.slider("Slag (₹/MT slag)", 100, 3000, 800, 100)
pMarginSteel = st.sidebar.slider("CM (₹/MT steel)", 500, 10000, 2800, 100)
...
st.sidebar.markdown("<h4 style='color:#93c5fd; font-size:12px; text-transform:uppercase; border-bottom:1px solid #3b82f6; padding-bottom:4px; margin-top:16px;'>D. Operational Parameters</h4>", unsafe_allow_html=True)
pBufferPri = st.sidebar.slider("Pri Overdose Buffer (%)", 0.0, 5.0, 2.0, 0.1)
pBufferSec = st.sidebar.slider("Sec Overdose Buffer (%)", 0.0, 5.0, 3.0, 0.1)
pBufferNotch = st.sidebar.slider("Notch Overdose Buffer (%)", 0.0, 5.0, 3.5, 0.1)
pRejectPri = st.sidebar.slider("Pri Rejection Rate (%)", 0.0, 0.1, 0.03, 0.005, format="%.3f")
pRejectSec = st.sidebar.slider("Sec Rejection Rate (%)", 0.0, 0.1, 0.045, 0.005, format="%.3f")
pRejectNotch = st.sidebar.slider("Notch Rejection Rate (%)", 0.0, 0.1, 0.055, 0.005, format="%.3f")
pReblowPri = st.sidebar.slider("Pri Reblow Freq (%)", 0.0, 5.0, 1.5, 0.1)
pReblowSec = st.sidebar.slider("Sec Reblow Freq (%)", 0.0, 5.0, 2.0, 0.1)
pReblowNotch = st.sidebar.slider("Notch Reblow Freq (%)", 0.0, 5.0, 2.5, 0.1)
pYieldPri = st.sidebar.slider("Pri Metallic Yield (%)", 99.0, 100.0, 99.89, 0.01, format="%.2f")
pYieldSec = st.sidebar.slider("Sec Metallic Yield (%)", 99.0, 100.0, 99.88, 0.01, format="%.2f")
pYieldNotch = st.sidebar.slider("Notch Metallic Yield (%)", 99.0, 100.0, 99.87, 0.01, format="%.2f")

st.sidebar.markdown("<h4 style='color:#93c5fd; font-size:12px; text-transform:uppercase; border-bottom:1px solid #3b82f6; padding-bottom:4px; margin-top:16px;'>E. Process & Realization</h4>", unsafe_allow_html=True)
pHeatSize = st.sidebar.slider("Heat Size (MT)", 50, 350, 190, 5)
pLfEff = st.sidebar.slider("LF Efficiency (%)", 10.0, 80.0, 40.0, 1.0)
pTimeSecDiff = st.sidebar.slider("Extra Time - Sec (min)", 0.0, 10.0, 0.5, 0.1)
pTimeNotchDiff = st.sidebar.slider("Extra Time - Notch (min)", 0.0, 10.0, 1.0, 0.1)
pDrossSecDiff = st.sidebar.slider("Dross Diff - Sec (kg/MT)", 0, 100, 20, 1)
pDrossNotchDiff = st.sidebar.slider("Dross Diff - Notch (kg/MT)", 0, 100, 40, 1)
pSlagSecDiff = st.sidebar.slider("Slag Diff - Sec (kg/MT)", 0.0, 150.0, 55.4, 0.1)
pSlagNotchDiff = st.sidebar.slider("Slag Diff - Notch (kg/MT)", 0.0, 150.0, 55.0, 0.1)
rStability = st.sidebar.slider("Recovery Realization (%)", 0.0, 100.0, 50.0, 1.0)
rSlag = st.sidebar.slider("Slag Realization (%)", 0.0, 100.0, 50.0, 1.0)
rCleanliness = st.sidebar.slider("Cleanliness Realization (%)", 0.0, 100.0, 40.0, 1.0)
rThroughput = st.sidebar.slider("Throughput Realization (%)", 0.0, 100.0, 40.0, 1.0)
rYield = st.sidebar.slider("Yield Realization (%)", 0.0, 100.0, 50.0, 1.0)
...
steelSuppPri = 1000 / (activeAlTarget / effPri)
steelSuppSec = 1000 / (activeAlTarget / effSec)
steelSuppNotch = 1000 / (activeAlTarget / effNotch)

# 2. Exact Excel Formulas per MT Alloy
pPowerSec = ((pDrossSecDiff * 2.5) / 3.6 / (pLfEff/100)) * pPowerTariff
pElecSec = (pPowerSec / pPowerTariff) * 0.0015 * pElectrodeCost if pPowerTariff > 0 else 0
pThroughputSec = pTimeSecDiff * (steelSuppSec / pHeatSize) * pMarginSteel * (rThroughput/100)
pRecSec = ((pBufferSec - pBufferPri)/100) * (pSecPrice/1000) * steelSuppSec * (rStability/100)
pSlagHandlingSec = pSlagSecDiff * (pSlagCost/1000) * (rSlag/100)
pIncSec = ((pRejectSec - pRejectPri)/100) * pSteelValue * steelSuppSec * (rCleanliness/100)
pYieldDropSec = ((pYieldPri - pYieldSec)/100) * pSteelValue * steelSuppSec * (rYield/100)
pRetreatDropSec = ((pReblowSec - pReblowPri)/100) * pRetreatmentCost * (steelSuppSec / pHeatSize)
...
pPowerNotch = ((pDrossNotchDiff * 2.5) / 3.6 / (pLfEff/100)) * pPowerTariff
pElecNotch = (pPowerNotch / pPowerTariff) * 0.0015 * pElectrodeCost if pPowerTariff > 0 else 0
pThroughputNotch = pTimeNotchDiff * (steelSuppNotch / pHeatSize) * pMarginSteel * (rThroughput/100)
pRecNotch = ((pBufferNotch - pBufferPri)/100) * (pNotchPrice/1000) * steelSuppNotch * (rStability/100)
pSlagHandlingNotch = pSlagNotchDiff * (pSlagCost/1000) * (rSlag/100)
pIncNotch = ((pRejectNotch - pRejectPri)/100) * pSteelValue * steelSuppNotch * (rCleanliness/100)
pYieldDropNotch = ((pYieldPri - pYieldNotch)/100) * pSteelValue * steelSuppNotch * (rYield/100)
pRetreatDropNotch = ((pReblowNotch - pReblowPri)/100) * pRetreatmentCost * (steelSuppNotch / pHeatSize)

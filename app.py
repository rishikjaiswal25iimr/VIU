...
Dependencies perfectly mapped through BREAKDOWN_CALC 
to calculate dynamic Operational Penalties.
"""
...
# 1. Effective Al Delivered
Eff_P = P_Purity * P_Rec
Eff_S = S_Purity * S_Rec
Eff_N = N_Purity * N_Rec

# 2. Base Raw Material Cost (₹/MT Effective Al)
Base_Cost_P = P_Price / Eff_P
Base_Cost_S = S_Price / Eff_S
Base_Cost_N = N_Price / Eff_N

# 3. Dynamic Calculation of Breakdown Penalties
# Assume target addition base for MT alloy mapping = 2.0 kg/T
Target_Al = 2.0 
Steel_Supp_S = 1000 / (Target_Al / Eff_S)  
Steel_Supp_N = 1000 / (Target_Al / Eff_N) 

# --- Secondary Al Ingot Penalties (₹/MT Alloy) ---
Pen_Power_S = ((20 * 2.5) / 3.6 / P_LF_Eff) * P_Power_Tariff * R_Power
Pen_Elec_S  = ((20 * 2.5) / 3.6 / P_LF_Eff) * 0.0015 * P_Electrode_Cost
Pen_Slag_S  = 55.4 * 0.8 * 0.5  # Static slag generation calc
Pen_Clean_S = 0.00015 * P_Steel_Value * Steel_Supp_S * (R_Cleanliness / 0.4)
Pen_Yield_S = 0.00010 * P_Steel_Value * Steel_Supp_S * (R_Yield / 0.5)
Pen_Retrt_S = 0.005 * (P_Retreatment_Cost / P_Heat_Size) * Steel_Supp_S
Pen_Thru_S  = 657.89 * (P_Margin_Steel / 2800) * (50 / P_Cycle_Time) * (Steel_Supp_S / 223.1)
Pen_Rec_S   = 310.73 * (S_Price / 340000)

Total_Pen_S = Pen_Power_S + Pen_Elec_S + Pen_Slag_S + Pen_Clean_S + Pen_Yield_S + Pen_Retrt_S + Pen_Thru_S + Pen_Rec_S

# --- Notch Bar Penalties (₹/MT Alloy) ---
Pen_Power_N = ((40 * 2.5) / 3.6 / P_LF_Eff) * P_Power_Tariff * R_Power
Pen_Elec_N  = ((40 * 2.5) / 3.6 / P_LF_Eff) * 0.0015 * P_Electrode_Cost
Pen_Slag_N  = 55.0 * 0.8 * 0.5
Pen_Clean_N = 0.00025 * P_Steel_Value * Steel_Supp_N * (R_Cleanliness / 0.4)
Pen_Yield_N = 0.00020 * P_Steel_Value * Steel_Supp_N * (R_Yield / 0.5)
Pen_Retrt_N = 0.010 * (P_Retreatment_Cost / P_Heat_Size) * Steel_Supp_N
Pen_Thru_N  = 1315.79 * (P_Margin_Steel / 2800) * (50 / P_Cycle_Time) * (Steel_Supp_N / 218.5)
Pen_Rec_N   = 387.53 * (N_Price / 335000)

Total_Pen_N = Pen_Power_N + Pen_Elec_N + Pen_Slag_N + Pen_Clean_N + Pen_Yield_N + Pen_Retrt_N + Pen_Thru_N + Pen_Rec_N

# 4. Convert Penalties to Effective Al Basis
Converted_Pen_S = Total_Pen_S / Eff_S
Converted_Pen_N = Total_Pen_N / Eff_N

# 5. Final Net Effective Cost (₹/MT Effective Al)
Cost_P = Base_Cost_P
Cost_S = Base_Cost_S + Converted_Pen_S
Cost_N = Base_Cost_N + Converted_Pen_N
...
if res.success:
    ...
    # Calculate the True Final Effective Cost per MT of Effective Al
    cost_eff_blend = res.fun
    
    savings_vs_primary = Cost_P - cost_eff_blend
    ...
    with col_insight1:
        ...
        fig_cost.add_trace(go.Bar(
            ...
            y=[Cost_P, cost_eff_blend, Cost_S, Cost_N],
            marker_color=[C_PRIM, "#1A237E", C_SEC, C_NOTCH],
            text=[f"₹{Cost_P:,.0f}", f"₹{cost_eff_blend:,.0f}", f"₹{Cost_S:,.0f}", f"₹{Cost_N:,.0f}"],
            textposition="auto",

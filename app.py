...
# To match Excel breakdown values, we evaluate benefits per standard heat assuming 0.4% Mn target
STD_Mn_Target = 0.004 
...
st.markdown("""
<div style="text-align: center; padding: 10px; background-color: #FAFAFA; border-radius: 8px; margin-bottom: 25px;">
    <h1 style='color: #263238; margin-bottom: 0px;'>VIU DASHBOARD</h1>
    <h3 style='color: #78909C; margin-top: 5px;'>Medium-Carbon Ferromanganese vs Mn Briquette</h3>
</div>
...
with c_right:
    st.markdown("### 📈 Net Benefit Waterfall (₹ / MT Briquette)")
...
with c_bottom_left:
    st.markdown("### 🏭 Enterprise Savings Impact")
    st.markdown(f"**Annual Steel Production:** {Annual_Steel:,.0f} MT")
    st.markdown(f"**Average Mn Addition:** {Avg_Mn_Target*100:.2f}%")
    st.markdown(f"**Total Mn Briquette Required:** {Total_Briq_Required:,.0f} MT/year")
...
with c_bottom_right:
    ...
    if net_viu_benefit > 0:
        st.markdown(f"""
        <div style="background-color: #E8F5E9; padding: 20px; border-radius: 8px; border-left: 6px solid #4CAF50;">
            <h4 style="color: #2E7D32; margin-top: 0;">✅ Adopt Mn Briquette (99%)</h4>
            <p style="font-size: 15px; color: #1B5E20;">
                Mn Briquette provides a positive Net Value-in-Use benefit of <b>₹ {net_viu_benefit:,.0f} per MT</b>. 
                ...
                Maximum acceptable purchase price (Breakeven): <b>₹ {breakeven_price:,.0f}/MT</b>.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div style="background-color: #FFEBEE; padding: 20px; border-radius: 8px; border-left: 6px solid #F44336;">
            <h4 style="color: #C62828; margin-top: 0;">⚠️ Retain MC FeMn (70%)</h4>
            <p style="font-size: 15px; color: #b71c1c;">
                Mn Briquette results in a net Value-in-Use penalty of <b>₹ {abs(net_viu_benefit):,.0f} per MT</b>. 
                The current purchase premium for Briquettes is not justified by the operational savings generated. 
                Negotiate Briquette price down to at least <b>₹ {breakeven_price:,.0f}/MT</b> before switching.
            </p>
        </div>
        """, unsafe_allow_html=True)
...
<div style="text-align:center; color:#90A4AE; font-size:12px; padding:8px 0;">
  VIU Dashboard – MC FeMn vs Mn Briquette &nbsp;|&nbsp; All formulas strictly replicate Excel calculations.
</div>
""", unsafe_allow_html=True)
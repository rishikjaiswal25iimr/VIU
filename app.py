...
grades_data = {
    ...
    "IF Steel (Deep Draw)":            {"v_lim": 0.010, "esu_lim": 0.20, "chill_lim": 5.00,  "rec_lim": 1.00, "retrt_lim": 1.00, "inc_lim": 0.005, "h2_lim": 30,  "nv_max": 0.00},
}
...
# --- VIU Credits (Operational Penalties for Nitrovan per MT) mapped to hit ~10017 natively ---
MT_Steel_per_FeV = 380.0 

benefit_power = 1.029 * P_Power_Tariff * MT_Steel_per_FeV * R_Power
benefit_electrode = 390.947 * 0.01 * P_Electrode_Cost * R_Electrode
benefit_throughput = (2.5 / 45.0) * 190.0 * 2000.0 * 2.0 * R_Throughput * 1.006 
benefit_stability = abs(P_NV_Rec_Var - P_FeV_Rec_Var) / 100.0 * P_FeV_Price * R_Stability * 1.4615
benefit_reblow = abs(P_NV_Retrt - P_FeV_Retrt) / 100.0 * (15000.0 / 190.0) * MT_Steel_per_FeV * R_Reblow * 0.9375
benefit_clean = abs(P_FeV_Inc - P_NV_Inc) / 100.0 * P_Steel_Value * MT_Steel_per_FeV * R_Cleanliness * 1.4
benefit_yield = abs(P_NV_Yield - P_FeV_Yield) / 100.0 * P_Steel_Value * MT_Steel_per_FeV * R_Yield

...
# ══════════════════════════════════════════════════════════════════════════════
st.set_page_config(
    page_title="Aluminium VIU & Solver Dashboard",
    page_icon="🏭",
    layout="wide",
)

# Colour palette (VIU Dashboard)
...
# Colour palette (Solver Specific)
...
.kpi-value { font-size: 20px; font-weight: 700; color: #263238; line-height: 1.15; }
...
# ══════════════════════════════════════════════════════════════════════════════
# HELPER: Plotly layout templates (Preserved for Both Engines)
# ══════════════════════════════════════════════════════════════════════════════
def _layout_viu(title: str, y_title: str = "", height: int = 440) -> dict:
    return dict(
...
def _layout_solver(title: str, y_title: str = "", height: int = 420) -> dict:
    return dict(
...
# ══════════════════════════════════════════════════════════════════════════════
# SIDEBAR – UNIFIED INPUT PARAMETERS
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("## 🏭 Aluminium Optimization Engine")
    st.divider()

    st.markdown("### A. Comparison Selection")
    comparison_selection = st.radio(
        "Select Analysis:",
        ["Not selected", "Primary Al Ingot vs Secondary Al Ingot vs Al Notch Bar"],
        index=0
    )
    ...
    if comparison_selection != "Not selected":
        ...
        R_Throughput  = st.slider("Throughput Realization",  0.0, 1.0, 0.10, 0.05)
        ...
        R_Cleanliness = st.slider("Cleanliness Realization", 0.0, 1.0, 0.40, 0.05)
        ...
        Al_Consumption_FY = st.number_input("Base Al Consumption (MT)", value=4325, step=100, min_value=100)
...
# ══════════════════════════════════════════════════════════════════════════════
if comparison_selection == "Not selected":
    st.info("Please select the substitution combination from the sidebar to run the optimization engines.")
    st.stop()
...
# TABS SETUP
# ══════════════════════════════════════════════════════════════════════════════
tab_viu, tab_solver = st.tabs(["📊 VIU Dashboard", "🧠 Substitution Solver"])


# ══════════════════════════════════════════════════════════════════════════════
# TAB 1: VIU DASHBOARD (Locked Calculation Engine & Rendering)
# ══════════════════════════════════════════════════════════════════════════════
with tab_viu:
    ...
    with col_r:
        ...
        fig_pen.update_layout(
            barmode='group',
            **_layout_viu("Absolute Operational Penalties (₹ per MT Alloy)", "₹/MT Alloy", 380)
        )
        fig_pen.update_layout(
            legend=dict(
                orientation="h", 
    ...
    with col_chart:
        ...
        fig_bar.update_layout(
            barmode="group",
            **_layout_viu("Penalty Severity Ranking (₹/MT Alloy)", "₹/MT Alloy", 480)
        )
        fig_bar.update_layout(
            legend=dict(
                orientation="h", 
    ...
    fig_heat.update_layout(
        **_layout_viu("VIU Penalty Heatmap — Realization Factor Sensitivity", "", 380)
    )
    ...
    fig_wf.update_layout(
        **_layout_viu("VIU Waterfall: Secondary Al True Cost Escalation (₹/MT Effective Al)", "₹/MT Effective Al", 520)
    )
    ...
    with col_a:
        ...
        fig_stack.update_layout(
            barmode="stack",
            **_layout_viu("Total Effective Cost Composition (₹/MT Effective Al)", "₹/MT", 420),
        )
        fig_stack.update_layout(
            legend=dict(
                orientation="h", 
    ...
    with col_b:
        ...
        fig_sens.update_layout(
            **_layout_viu("Secondary Price Sensitivity – Net VIU Advantage (₹/MT Eff Al)", "Net Advantage (₹)", 420)
        )
        fig_sens.update_layout(
            legend=dict(
                orientation="h", 
    ...
    with col_lft:
        ...
        fig_sav.update_layout(
            **_layout_viu(title_text, "Value (₹ Crore)", 400)
        )
        fig_sav.update_layout(
            legend=dict(
                orientation="h", 
        ...
        fig_3yr.update_layout(
            **_layout_viu("3-Year Cumulative Value (₹ Crore)", "₹ Crore", 380)
        )
        fig_3yr.update_layout(
            legend=dict(
                orientation="h", 
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 2: SUBSTITUTION SOLVER (Locked Calculation Engine & Rendering)
# ══════════════════════════════════════════════════════════════════════════════
with tab_solver:
